...
if (!class_exists('VG_Visual_Composer_Integration')) {
	...
	class VG_Visual_Composer_Integration {
		...
		function init() {
			...
			add_action('vg_sheet_editor/columns/provider_items', array($this, 'filter_columns_settings'), 10, 2);
		}
		...
		function filter_columns_settings($spreadsheet_columns, $post_type) {
			...
			if (isset($spreadsheet_columns['content'])) {
				$spreadsheet_columns['content']['column_width'] = 230;
			}
			if (isset($spreadsheet_columns['post_content'])) {
				$spreadsheet_columns['post_content']['column_width'] = 230;
			}
			return $spreadsheet_columns;
		}
